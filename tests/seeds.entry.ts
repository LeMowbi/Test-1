...
check(
  seedPlayers.every((p) => p.level >= 1 && p.level <= 7),
  'Niveaux joueurs ∈ [1.0, 7.0]',
);
check(
  seedFriends.every((f) => f.level === undefined || (f.level >= 1 && f.level <= 7)),
  'Niveaux amis ∈ [1.0, 7.0]',
);
check(
  seedPlayers.some((p) => p.level === 1.0),
  'Un seed à 1.0 existe (test du plancher)',
);
